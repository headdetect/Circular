...
namespace Circular.Display.Screens {
    ...
    public class MenuScreen : GameScreen {
        ...
        public void AddLevelItem ( LevelBase level ) {
            MenuEntry entry = new MenuEntry( this, level.GetTitle(), EntryType.Screen, level, ContentHelper.GetTexture("logo") );
            _menuEntries.Add( entry );
        }
        ...
        public override void HandleInput ( InputHelper input, GameTime gameTime ) {
            ...
            if ( input.IsMenuSelect() && _selectedEntry != -1 ) {
                var item = _menuEntries[ _selectedEntry ];
                if ( item.IsExitItem() ) {
